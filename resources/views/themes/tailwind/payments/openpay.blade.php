...
{{-- Modal --}}
<div x-data="AlpineModalPlans()" x-init="init()" @alpine-modal-plans.window="selectProduct($event.detail.plan_id, 'plan')">
        <!-- Modal Background -->
        <div x-show="open" class="fixed text-gray-500 flex items-center justify-center overflow-y-auto z-50 bg-black bg-opacity-40 left-0 right-0 top-0 bottom-0" x-transition:enter="transition ease duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            <!-- Modal -->
            <div x-show="open" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 mx-10" @click.away="open = false" x-transition:enter="transition ease duration-100 transform" x-transition:enter-start="opacity-0 scale-90 translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease duration-100 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-1">
                <livewire:openpay-payment-livewire />
            </div>
        </div>
</div>
    {{--
...
<script>
    ...
    var deviceSessionId = OpenPay.deviceData.setup(document.getElementById('openpay-form'), "deviceIdHiddenFieldName");
    ...
    {
        Livewire.emit('setProduct', productId);
    }
    ...
    window.addEventListener('payment-event', event => {
        console.log('Name updated to: ' + event.detail.newName);
        let OpenPayForm = document.getElementById('openpay-form');
        OpenPay.token.extractFormAndCreate(OpenPayForm, dumpComplete, dumpComplete, '@if(!auth()->guest()){{ auth()->user()->email }}@endif');
    })

    function dumpComplete(response)
    {
        console.log(response);
        Livewire.emit('token', response);
    }
</script>
